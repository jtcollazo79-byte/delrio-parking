...
document.getElementById("exportCSV").addEventListener("click", () => {
  ...
  const headers = ["Espacio","Tenant","Placa","Vehículo","Tipo","Fecha","Hora","Oficial","Estado","Multa","Notas","Fotos"];
  const rows = allInfractions.map(inf => {
    ...
    const multa = (vs === "Se Quedó") ? "$100.00" : "";
    return [spaceNum, tenantName, inf.plate||"", inf.vehicle||"", inf.type||"", date, time, officerName, vs, multa, inf.notes||"", photos].map(v => `"${v}"`).join(",");
  });
...
document.getElementById("exportPDF").addEventListener("click", () => {
  ...
  html += `<table><tr><th>Fecha</th><th>Espacio</th><th>Tenant</th><th>Placa</th><th>Vehículo</th><th>Tipo</th><th>Hora</th><th>Oficial</th><th>Estado</th><th>Multa</th><th>Notas</th><th>Fotos</th></tr>`;
  allInfractions.forEach(inf => {
    ...
    const multa = (vs === "Se Quedó") ? "$100.00" : "—";
    html += `<tr><td>${dateOnly}</td><td>${spaceNum}</td><td>${inf.tenant||""}</td><td>${inf.plate||""}</td><td>${inf.vehicle||""}</td><td>${inf.type||""}</td><td>${time}</td><td>${officerName}</td><td>${vs}</td><td>${multa}</td><td>${inf.notes||""}</td><td>${photoImgs}</td></tr>`;
  });
